                    ...
                    {workout.overallNotes && (
                      <div className="mt-3 pt-2 border-t border-gray-100">
                        <h4 className="text-xs font-semibold text-gray-600 mb-1 flex items-center">
                          <Edit3 size={12} className="mr-1.5 text-blue-500" /> Session Notes
                        </h4>
                        <p className="text-xs text-gray-500 whitespace-pre-wrap bg-gray-50 p-2 rounded-md">{workout.overallNotes}</p>
                      </div>
                    )}

                    <div className="mt-3 space-y-1 text-xs sm:text-sm">
                      <div className="grid grid-cols-3 gap-x-2 px-2 py-1 font-semibold text-gray-600 bg-gray-100 rounded-t-md">
                        <div>Exercise</div>
                        <div className="text-center">Sets</div>
                        <div className="text-right">Best Set</div>
                      </div>
                      {workout.exercises.map(ex => (
                        <React.Fragment key={ex.id}>
                          <div className="grid grid-cols-3 gap-x-2 px-2 py-1.5 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 rounded-sm">
                            <div className="truncate text-gray-700 col-span-1" title={ex.name}>{ex.name}</div>
                            <div className="text-center text-gray-600 col-span-1">{ex.loggedSets.filter(s => s.completed).length}</div>
                            <div className="text-right text-gray-600 font-medium col-span-1">{findBestSet(ex)}</div>
                          </div>
                          {ex.notes && (
                             <div className="px-2 pb-1.5 pt-0.5 border-b border-gray-100 last:border-b-0 bg-blue-50 hover:bg-blue-100 rounded-sm -mt-px">
                              <p className="text-xs text-blue-700 whitespace-pre-wrap flex items-start">
                                  <Edit3 size={11} className="mr-1.5 mt-0.5 flex-shrink-0" /> 
                                  <span>{ex.notes}</span>
                              </p>
                             </div>
                          )}
